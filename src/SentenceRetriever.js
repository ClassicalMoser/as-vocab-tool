...
//this seems unnecessary since it's been moved to the backend
...
/*const TABLE = {
    ...
    //this isn't used to my knowledge
    VOCABULARY_EXAMPLES: 'brrcdgykk'
}*/


//this function is too big and contains various things,
//some of which I do need but more of which I don't
export default function SentenceRetriever() {
//  const [userToken, setUserToken] = useState('')//no longer necessary since it's been moved to the backend

    const [vocabTable, setVocabTable] = useState([])//still needed since it directly affects react
    const [examplesTable, setExamplesTable] = useState([])//still needed since it directly affects react
    const [lessonsTable, setLessonsTable] = useState([])//still needed since it directly affects react
    ...
    // new function
    //Most of this is unnecessary since it's been moved to the backend. See comments for the three lines still needed
    /*
    function createLocalTable(tableName, jsonData) {
        ...
        switch(tableName) {
            ...
            case TABLE.VOCABULARY.NAME:
                ...
                setVocabTable(newArr)//These are the three functions that are still needed, but should fetch directly from backend.
                return
            case TABLE.EXAMPLES.NAME:
                ...
                setExamplesTable(newArr)//These are the three functions that are still needed, but should fetch directly from backend.
                return
            ...
            case TABLE.LESSONS.NAME:
                ...
                //console.log(newArr)
                setLessonsTable(newArr)//These are the three functions that are still needed, but should fetch directly from backend.
            default:
                //
        }
    }
    */
    ...
    //No longer needed since backend doesn't require headers
    ...
    //combined version of vocab & examples
    //Entirely unneeded. Cloned entirely on backend
    /*
    ...
    // This has also been moved entirely to the backend so the call should be put where the table state is set instead. Keep fetch-then-catch structure
    /*
